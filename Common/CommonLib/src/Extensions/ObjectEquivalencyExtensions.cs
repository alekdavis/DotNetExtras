...
public static class ObjectEquivalencyExtensions
{
    /// <summary>
    /// Checks if the source object is identical to the target (comparing all public properties and fields). 
    /// </summary>
    /// <param name="source">
    ...
    )
    {
        ...
        else if (source is bool && (target is short || target is int || target is long))
        {
        ...
        else if (target is bool && (source is short || source is int || source is long))
        {
        ...
        else if (sourceType.IsClass && targetType.IsClass)
        {
            ...
            foreach (PropertyInfo property in sourceType.GetProperties(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance))
            {
                if (property.CanRead)
                {
            ...
            if (!ignoreNullSource)
            {
                foreach (PropertyInfo property in targetType.GetProperties(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance))
                {
                    if (property.CanRead)
                    {
            ...
            foreach (FieldInfo field in sourceType.GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance))
            {
                skip.Add(field.Name);
